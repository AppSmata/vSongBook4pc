...
bool vSongBook::OnInit()
{
	...
	vSongHome* home = new vSongHome("vSongBook for Desktop!");
...
void vSongHome::PopulateSongbooks()
{
	try 
	{
		if (cmbSongBooks->GetCount() > 0) cmbSongBooks->Clear();
...
{
	int setsong = lstSongList->GetSelection();
	...
	*/
	OpenSongPreview(setsong);
}

void vSongHome::OpenSongPreview(int setsong)
{
	TxtSongTitle->SetValue(wxString::FromUTF8(songtitles[setsong] + " | " + songbooks[setsong]));
	TxtPreview->SetValue(AppSmata::ContentText(songcontents[setsong]));
	TxtExtras->SetValue(wxString::FromUTF8(songaliases[setsong]));
}
