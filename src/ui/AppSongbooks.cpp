...
void AppSongbooks::LoadBooklist(QString searchstr)
{
	...
	QByteArray bar = AsUtils::BookSearchSql(searchstr).toLocal8Bit();
    char* sqlQuery = bar.data();

    int row, col;
    int rc = sqlite3_open_v2(AsUtils::DbNameChar(), &db, SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE, NULL);
...
void AppSongbooks::LoadBook()
{
	...
	QByteArray bar = AsUtils::BookSingleSql(Songbookid).toLocal8Bit();
	char* sqlQuery = bar.data();
...
void AppSongbooks::SaveChanges()
{
	...
	QByteArray bar = AsUtils::BookUpdateSql(Songbookid, Title, Tags, Content, Songcount).toLocal8Bit();
	char* sqlQuery = bar.data();
...
void AppSongbooks::DeleteBook()
{
	...
	rc = sqlite3_exec(db, AsUtils::BookDeleteSql(Songbookid), 0, 0, &zErrMsg);
