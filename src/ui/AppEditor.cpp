...
void AppEditor::LoadSong()
{
	...
	QByteArray bar = AsUtils::SongSingleSql(song_id).toLocal8Bit();
	char* sqlQuery = bar.data();
	rc = sqlite3_get_table(songsDb, sqlQuery, &qryResult, &row, &col, &err_msg);
...
void AppEditor::SaveChanges()
{
	...
	QByteArray bar = AsUtils::SongUpdateSql(Number, Title, Alias, Content, Key, Author, song_id).toLocal8Bit();
	char* sqlQuery = bar.data();
...
void AppEditor::DeleteSong()
{
	...
	rc = sqlite3_exec(db, AsUtils::SongDeleteSql(song_id), 0, 0, &zErrMsg);
