...
# Install SQLite3 
# Note - `brew tap vsongbook/homebrew-sqlite3` needs to have been run at least once (manually) first
echo Install SQLite3 >>$LOG 2>&1
$BREW install sqlitefts5 >>$LOG 2>&1

# Update the vsongbook source
echo Update the vsongbook source >>$LOG 2>&1
cd $HOME/git_repos/vsongbook >>$LOG 2>&1
...
# Build and package standard vsongbook nightly
echo Build and package standard vsongbook nightly >>$LOG 2>&1
if [ "${BUILD_TYPE}" = "debug" ]; then
  $QMAKE vsongbook.pro -r -spec macx-clang CONFIG+=debug CONFIG+=x86_64 >>$LOG 2>&1
else
  $QMAKE vsongbook.pro -r -spec macx-clang CONFIG+=x86_64 >>$LOG 2>&1
fi
make -j3 >>$LOG 2>&1
...
# Clean the vsongbook source
echo Clean the vsongbook source >>$LOG 2>&1
cd $HOME/git_repos/vsongbook >>$LOG 2>&1
git reset --hard HEAD >>$LOG 2>&1
git clean -dffx >>$LOG 2>&1
git checkout $BRANCH >>$LOG 2>&1
...
# Build and package vsongbook with SQLCipher support
echo Build and package vsongbook with SQLCipher support >>$LOG 2>&1
if [ "${BUILD_TYPE}" = "debug" ]; then
  $QMAKE vsongbook.pro -r -spec macx-clang CONFIG+=debug CONFIG+=x86_64 CONFIG+=sqlcipher >>$LOG 2>&1
else
  $QMAKE vsongbook.pro -r -spec macx-clang CONFIG+=x86_64 CONFIG+=sqlcipher >>$LOG 2>&1
fi
make -j3 >>$LOG 2>&1
make -j3 >>$LOG 2>&1 # Seems to need a 2nd time now, due to language files needing initialisation or something
